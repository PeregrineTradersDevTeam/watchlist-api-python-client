...
If you decide to run the Watchlist API Client Library for Python within a Conda virtual environment, make sure to:
...
- `cd` to the Watchlist API Client Library source directory (the directory where the repository was cloned).
...
conda activate watchlist-client-prod-env
...
# Use an ENV_DIR of your choice. We will use ~/virtualenvs/wac-prod
# Any parent directories should already exist
python3 -m venv ~/virtualenvs/wac-prod

# Activate the virtualenv
. ~/virtualenvs/wac-prod/bin/activate
...
```shell
# Create a virtual environment
# Use an ENV_DIR of your choice. We will use ~/Users/<yourname>/.pyenv/versions/wac-prod
# To specify a pyenv-virtualenv environment we use: pyenv virtualenv <version> <env-name>
pyenv virtualenv 3.8.6 wac-prod

# Activate the virtualenv
pyenv activate wac-prod
...
python -m venv $env:USERPROFILE\virtualenvs\wac-prod
...
- Test on Linux
- Include pip requirements generated with `pip-tools`
...
Watchlist API Client Library for Python is developed and maintained by [Jacopo Abbate](mailto:jacopo.abbate@peregrinetraders.com "jacopo.abbate@peregrinetraders.com").